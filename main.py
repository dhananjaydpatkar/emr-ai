...
def main():
    ...
    fhir_server = "http://mock-fhir-server.org"
    initial_agent = InitialAgent(fhir_server_url=fhir_server)
    extractor_agent = ExtractorAgent()
    reconciler_agent = ReconcilerAgent()
    validator_agent = ValidatorAgent()
    outbound_agent = OutboundAgent(fhir_server_url=fhir_server)

    # 2. Pipeline Execution
    try:
        # Step 1: Initial Agent (Context)
        print("\n[Step 1] Initial Agent: Fetching Patient Context...")
        patient_context = initial_agent.get_patient_context(args.patient_id)
        ...
        outbound_agent.save_bundle(min_validated_bundle)
